...
def run_test_pizza():
    ...
    # Test 4
    title = 'large number of black lines on yellow circle'
    window = rg.RoseWindow(400, 400, title)
    circle = rg.Circle(rg.Point(200, 200), 175)
...
def pizza(window, circle, number_of_slices, color, thickness):
    ...
    circle.attach_to(window)
    slice = generate_points_on_circle(circle, number_of_slices)
...
def run_test_polygon():
    ...
    polygon(window, circle, 10, 'green', 1)
...
def run_test_fancy_polygon():
    ...
    fancy_polygon(window, circle, 10, 2, 'black', 5)
    window.close_on_mouse_click()
...
def fancy_polygon(window, circle, number_of_lines, hops_to_next_point, color, thickness):
    ...
    for k in range(len(points)):
        if k + hops_to_next_point < len(points):
            ...
            newpoint = (k + hops_to_next_point) - len(points)
            start = points[k]
            end = points[newpoint]
            line = rg.Line(start, end)
            line.color = color
